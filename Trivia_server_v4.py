...
"""
 new exception class EmptyMsgRecieved() 
"""
##############################################################################
import select
import socket
...
class EmptyMsgRecieved(Exception):
	"""exception raised when the remote socket closed and so the data recived is empty"""
	def __init__(self):
		super().__init__()
...
def recv_message_and_parse(conn : socket.socket):
	...
	try:
		...
		print(f"A client disconnected unexpectedly and removed from the list.")
		client_sockets.remove(conn)
		try:
			conn.close()  # Ensure socket is closed
		except OSError:
	...
	while True:
		...
		for current_socket in ready_to_read:
			if current_socket is ServerSocket:
				...
				except:  # Handle unexpected error
...
"""
when the client shutdown unexpectetdly 
"""
