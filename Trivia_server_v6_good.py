...
# server.py
"""
 dispose_dead_client() edited to overcome the unexpected leaving of client by itself
 client_sockets [] is still constant
"""
##############################################################################
import select
import socket
...
def dispose_dead_client(conn : socket.socket): # , client_sockets : list
	...
	try:
		...
		if conn in client_sockets:
			print(f"A client disconnected unexpectedly and has removed from the lists.")
...
def handle_logout_message(conn : socket.socket):
	...
	"""
	global client_sockets
	print(f"user {conn.getpeername()} logged out...")
	client_sockets.remove(conn)
	conn.close()
	# if conn is not None:
	# 	try:
...
def manage_to_handle_client_message(conn: socket.socket, cmd: str, data: Any) -> bool:
	"""
		...
		for current_socket in ready_to_read:
			if current_socket is ServerSocket:
				...
				# try:
					...
					if not manage_to_handle_client_message(current_socket, cmd, data):
						dispose_dead_client(current_socket) # , client_sockets
