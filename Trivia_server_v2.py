...
#***********************************************************************
def build_and_send_message(conn, code, msg) -> None:
	"""
...
def handle_logout_message(conn : socket.socket):
	...
	if conn is not None:
		try:
			...
			# Handle potential error if the socket is already closed
			pass
	...
	while True:
		...
		for current_socket in ready_to_read:
			if current_socket is ServerSocket:
				...
				except (ConnectionResetError, ConnectionAbortedError):  # Handle unexpected client disconnection
					print(f"Client {client_address} disconnected unexpectedly.")
					print("if you here its good", client_sockets)		#
...
"""
 use the keyword 'global' to import variable outsinde the scope 
...
"""
	...
	#  add to avoid server crushes when client is shutting down without .close() !!!!!!!!!!
